...
Raphael.fn.pieChart = function (width, height, cx, cy, r, stroke, font_size, flavor_input_id, flavor_index) {
	var paper = this,
		rad = Math.PI / 180,
		outer_rad_mult = 1.8,
		chart = this.set(),
	...
	var select_outer_item = function (tea_index) {
		...
		selected_teas[flavor_index] = json.categories[curr_category].category.teas[tea_index];
...
Raphael.fn.slider = function(width, height, num_values, font_size) {
	...
	var dragger = function (x, y, event) {
		...
		updatePercent = function() {
			...
			selected_teas_percentages[2] = 100.0;
			for (var i = 0; i < sliders.length; i++) {
				selected_teas_percentages[i] = (((sliders[i].getBBox().x - start)/slider_bar.getBBox().width)*100).toFixed(0);

				var input = document.getElementById(text_ids[i]);
				input.value = selected_teas_percentages[i]+"%";
				start = sliders[i].getBBox().x;
				selected_teas_percentages[2] -= selected_teas_percentages[i];
			}
			var input = document.getElementById(text_ids[text_ids.length-1]);
			input.value = selected_teas_percentages[2].toFixed(0)+"%";
			update_flavor_profile();
		},
...
function update_flavor_profile() {
	...
	for(var i = 0; i < keys.length ; i++) {
		...
		if (profiles[keys[i]] != 0) {
			...
			valueCell.innerHTML =  "<img src = './assets/star_" + profiles[keys[i]] + ".png' width='150' height='22'> <br />";
...
Raphael.fn.teaCups = function (width, height, cx, cy, r, stroke, font_size) {
	var paper = this,
		chart = this.set(),
		ms = 500,
		maxFlavors = 0,
	...
	var txt = paper.text(20, 20, "You can choose up to\nthree flavors for your blend.\nClick on the ones you want to add")
		.attr({fill: tcolor, stroke: "none", opacity: 0.5, "font-size": font_size});
...
window.onload = function () {
	...
	Raphael("holder1", width, height).pieChart(width, height, width/2, width/2, width/4, "#fff", font_size, "flavor1",0);
	Raphael("holder2", width, height).pieChart(width, height, width/2, width/2, width/4, "#fff", font_size, "flavor2",1);
	Raphael("holder3", width, height).pieChart(width, height, width/2, width/2, width/4, "#fff", font_size, "flavor3",2);
	Raphael("sliderbar", 920, 40).slider(920,40,3, font_size);
	Raphael("tea_cups_div", width, height).teaCups(width, height, width/2, width/2, width/4, "#fff", font_size);
};
