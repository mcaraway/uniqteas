  ...
  jQuery.fn.PaRSS = function(feed_url, item_count, date_format, show_descriptions, callback_function) {
    ...
    function getItems(entries){
      ...
      jQuery.each(entries, function(i, entry) {
        var list_item = "<div class='parss-title'><a href='" + entry.link + "'>" + entry.title + "</a></div>";

        if(data.date_format && data.date_format.length > 0) {
          list_item += "<div class='parss-date'>" + getFormattedDate(entry.publishedDate, data.date_format) + "</div>";
        }
        ...
        switch (data.show_descriptions) {
          case "image":
            ...
            if(img) {
              list_item += "<div class='parss-image'>" + img + "</div>";
            }
            list_item += "<div class='parss-description'>" + entry.contentSnippet + "</div>";
            break;
          case "content":
